...
describe("League Tests", () => {
  ...
  describe("League Creation", () => {
    it("Should create a league successfully", async () => {
      ...
      console.log("Using entry token mint:", accounts.entryTokenMint.toString());
      ...
      leaguePDA = globalTestState.createLeaguePDA(accounts.user1.publicKey, nonce);

      const rewardVaultPDA = await globalTestState.getRewardVaultATA(accounts.entryTokenMint, leaguePDA);

      console.log("League PDA:", leaguePDA.toString());
      console.log("Reward Vault PDA:", rewardVaultPDA.toString());
      console.log("Markets:", markets.map((market) => market.toString()));
      ...
      console.log("✅ League created successfully with correct entry token mint");
    });
  });
  ...
  describe("League Status Management", () => {
    ...
    it("Should fail to join league when status is not Active", async () => {
      ...
      participantPDA = globalTestState.createParticipantPDA(leaguePDA, accounts.user2.publicKey);
    ...
    it("Should fail for non-creator to start league before start time", async () => {
      console.log("❌ Testing start league failure for non-creator before start time...");
      ...
      try {
        ...
        console.log("✅ Correctly failed to start league for non-creator before start time");
      }
    });
    ...
    describe("League Joining", () => {
      ...
      it("Should allow user to join league after start", async () => {
        ...
        const amount = TEST_CONFIG.ENTRY_AMOUNT;
        participantPDA = globalTestState.createParticipantPDA(leaguePDA, accounts.user1.publicKey);
        ...
        expect(participant.user.toString()).to.equal(accounts.user1.publicKey.toString());
        expect(participant.virtualBalance.toString()).to.equal(TEST_CONFIG.VIRTUAL_BALANCE.toString()); // Default virtual balance
        expect(participant.claimed).to.be.false;

        console.log("✅ User successfully joined league with correct token usage");
      });
      ...
      it("Should fail for non-creator to close league before end time", async () => {
        ...
        try {
          await getProgram().methods
            ...
            .accounts({
              league: leaguePDA,
              user: accounts.user2.publicKey, // Non-creator
            } as any)
          ...
          console.log("✅ Correctly failed for non-creator to close league before end time");
        }
      });
